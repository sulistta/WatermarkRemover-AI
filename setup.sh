...
if [ "$CHINA_MODE" == "1" ]; then
    ...
    pip install "pywebview>=4.0" "qtpy>=2.4.1" "PyQt6>=6.6.0" "PyQt6-WebEngine>=6.6.0" --no-cache-dir $PIP_MIRROR -q
    ...
    pip install "pywebview>=4.0" "qtpy>=2.4.1" "PyQt6>=6.6.0" "PyQt6-WebEngine>=6.6.0" --no-cache-dir -q
...
# Do this before installing iopaint itself so pip does not warn about iopaint's full
# optional web UI dependency set during this targeted install.
